<template>
  <fieldset id="properties">
    ...
    <div v-for="name in getCategories()" :key="name">
      <div
        v-if="name != '_uncategorized'"
        class="edit_category_header"
      >{{categories[name].display_name}}</div>
      <table class="edit_category">
        <tbody>
          <tr v-for="row in getCategoryRows(name)">
            ...
            <td
              :id="'edit_' + row.name"
              v-html="getHtml(row.field)"
              :colspan="getColspan(row)"
              :class="{col1: !row.fullrow, fullrow: row.fullrow}"
            ></td>
            <th
              :id="'label_' + row.name2"
              v-html="getHtml(row.header2)"
              v-if="!row.fullrow"
              class="col2"
            ></th>
            <td
              :id="'edit_' + row.name2"
              v-html="getHtml(row.field2)"
              v-if="!row.fullrow"
              class="col2"
            ></td>
          </tr>
  ...
  methods: {
    ...
    getCategories: function() {
      return Object.keys(this.categories).sort(
        (a, b) => this.categories[a].index - this.categories[b].index
      );
    },
    getCategoryRows: function(name) {
      ...
      if (category.fields.indexOf("summary") >= 0) {
        this.fields["summary"].parent().attr("colspan", 3);
        result.push({
          header: this.headers["summary"],
          field: this.fields["summary"],
          name: "summary",
          fullrow: true
        });
      }
      ...
      for (i in category.fields) {
        ...
        if (
          field == "description" ||
          (this.fields[field] &&
            this.fields[field].parent().attr("colspan") == 3)
        ) {
          result.push({
