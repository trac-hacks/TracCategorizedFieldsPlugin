<template>
  <div>
    <div
      v-for="name in getCategories()"
      :key="name"
      :id="'cat_' + name"
      :class="{description: name != '_uncategorized'}"
    >
      <h2 v-if="name != '_uncategorized'">{{categories[name].display_name}}</h2>
        ...
        <tbody>
          <tr v-for="row in getCategoryRows(name)" :class="{fullrow: row.fullrow}">
            ...
            <th
              ...
              v-if="!row.fullrow"
              :class="getClass(row.header2)"
            ></th>
            <td
              :headers="'h_' + row.name2"
              v-html="getHtml(row.field2)"
              v-if="!row.fullrow"
              :class="getClass(row.field2)"
            ></td>
          </tr>
        </tbody>
      </table>
...
module.exports = {
  ...
  created: function() {
    this.element = $("#ticket-properties1");
